        ...
        <div className="dialog-body">
            {tab === 'pair' && <div className="form-stack">
                ...
                {draft.mode === 'existing' ? <div className="form-grid two"><Field label="Token address"><input className="mono" value={draft.tokenAddress || ''} onChange={e => set('tokenAddress', e.target.value)} placeholder="0x..."/></Field><Field label={draft.protocol === 'v2' ? 'Bonding curve address' : 'V3 pool address'}><input className="mono" value={draft.poolAddress || ''} onChange={e => set('poolAddress', e.target.value)} placeholder="0x..."/></Field></div> : <>
                    ...
                    {draft.protocol === 'v2' && <>
                        <label className="toggle-line"><input type="checkbox" checked={Boolean(draft.bundleBuys)} onChange={e => set('bundleBuys', e.target.checked)}/><span className="toggle"/><div><strong>Bundle maker buys with the launch (block-limited, no slippage)</strong><small>Pre-sign one buy per funded maker against the predicted curve address and broadcast them in the same batch as the launch through our block-limited router. Each buy fills only if it lands within the window below; otherwise it reverts and the wallet keeps its ETH.</small></div></label>
                        {draft.bundleBuys && <>
                            <NumberField label="Window (L2 blocks, ~100ms each)" value={draft.bundleMaxBlocks || 3} step={1} min={1} max={50} onChange={v => set('bundleMaxBlocks', Math.max(1, Math.round(v)))}/>
                            <div className="inline-note"><Gauge size={17}/><span>The window counts from the head height seen when the batch is sent, so it must cover your own latency to the sequencer: the run log prints "launch landed … blocks_after_send" — if that exceeds the window, every bundled buy reverts. Start at 3, then set the window to that figure plus 1–2. Requires the router contract in Settings (a default deployment is built in) and a wss:// RPC so the head height is pushed, not polled.</span></div>
                        </>}
                    </>}
...
function SettingsPage({initial, funding, vaultUnlocked, notify}: {
    ...
    return <section className="settings-layout"><div className="section-block"><div className="section-heading"><div><h2>Network</h2><p>Shared defaults for all configured pairs.</p></div></div><Field label="Robinhood Chain RPC"><div className="password-input"><input className="mono" type={showRPC ? 'text' : 'password'} value={settings.rpcEndpoint || ''} onChange={e => setSettings({...settings, rpcEndpoint: e.target.value})} placeholder="wss://..."/><button title={showRPC ? 'Hide endpoint' : 'Reveal endpoint'} onClick={() => setShowRPC(!showRPC)}>{showRPC ? <EyeOff size={17}/> : <Eye size={17}/>}</button></div></Field><div className="inline-note"><Gauge size={17}/><span>WebSocket endpoints enable event-driven pool monitoring. HTTPS falls back to polling.</span></div></div>
        <div className="section-block"><div className="section-heading"><div><h2>Block-limited router</h2><p>Our PonsMMRouter proxy used by launch bundles. Leave empty to use the built-in deployment.</p></div></div><Field label="Router contract address"><input className="mono" value={settings.mmRouter || ''} onChange={e => setSettings({...settings, mmRouter: e.target.value})} placeholder="0x1119cDed80b82CA4d732fD1bB20c13f5e9425F60 (default)"/></Field><div className="inline-note"><Gauge size={17}/><span>Bundled maker buys call buyWithin(curve, maxL2Block, wallet) here; the contract reverts once ArbSys.arbBlockNumber() passes maxL2Block, so a late buy never fills at a worse price.</span></div></div>
