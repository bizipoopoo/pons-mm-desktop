        ...
        <div className="data-table strategy-list">
            ...
            {(data.strategies || []).map(s => {
                ...
                return <div className="table-row" key={s.id}>
                    ...
                    <div><span className="mode-tag">{s.mode === 'launch' ? 'Launch' : 'Existing'}</span></div>
    ...
    return <Modal title={strategy.id ? `Edit ${strategy.name}` : 'New strategy'} subtitle="All execution parameters are stored locally." onClose={onClose} wide>
        ...
        <div className="dialog-body">
            {tab === 'pair' && <div className="form-stack">
                ...
                {draft.mode === 'existing' ? <div className="form-grid two"><Field label="Token address"><input className="mono" value={draft.tokenAddress || ''} onChange={e => set('tokenAddress', e.target.value)} placeholder="0x..."/></Field><Field label="V3 pool address"><input className="mono" value={draft.poolAddress || ''} onChange={e => set('poolAddress', e.target.value)} placeholder="0x..."/></Field></div> : <>
                    ...
                    <div className="form-grid two"><Field label="Fee wallet (optional)"><input className="mono" value={draft.token?.feeWallet || ''} onChange={e => setToken('feeWallet', e.target.value)} placeholder="Defaults to deployer"/></Field><Field label="Initial buy (ETH)"><input type="number" min="0" step="0.001" value={draft.devBuyEth ?? 0} onChange={e => set('devBuyEth', +e.target.value)}/></Field></div>
