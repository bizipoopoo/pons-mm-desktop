                ...
                {draft.mode === 'existing' ? <div className="form-grid two"><Field label="Token address"><input className="mono" value={draft.tokenAddress || ''} onChange={e => set('tokenAddress', e.target.value)} placeholder="0x..."/></Field><Field label={draft.protocol === 'v2' ? 'Bonding curve address' : 'V3 pool address'}><input className="mono" value={draft.poolAddress || ''} onChange={e => set('poolAddress', e.target.value)} placeholder="0x..."/></Field></div> : <>
                    ...
                    {draft.protocol === 'v2' && <>
                        ...
                        {draft.bundleMode === 'window' && <>
                            <NumberField label="Window (L2 blocks after the launch, ~100ms each)" value={draft.bundleMaxBlocks || 3} step={1} min={1} max={50} onChange={v => set('bundleMaxBlocks', Math.max(1, Math.round(v)))}/>
                            <div className="inline-note"><Gauge size={17}/><span>The launch runs through our router, which records the L2 block it lands in. One buy per funded maker is pre-signed against the predicted curve and sent in the same batch; the contract fills it only if it lands within this many blocks of the launch block, otherwise it reverts and the wallet keeps its ETH. No slippage bound. The router becomes the on-chain deployer; creator fees and tokens stay on your wallets. Buys landing in the same block as the launch count as 0.</span></div>
                        </>}
