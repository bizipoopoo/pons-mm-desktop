            ...
            {tab === 'pair' && <div className="form-stack">
                ...
                {draft.mode === 'existing' ? <div className="form-grid two"><Field label="Token address"><input className="mono" value={draft.tokenAddress || ''} onChange={e => set('tokenAddress', e.target.value)} placeholder="0x..."/></Field><Field label={draft.protocol === 'v2' ? 'Bonding curve address' : 'V3 pool address'}><input className="mono" value={draft.poolAddress || ''} onChange={e => set('poolAddress', e.target.value)} placeholder="0x..."/></Field></div> : <>
                    ...
                    {draft.protocol === 'v2' && <>
                        ...
                        {draft.bundleMode === 'atomic' && <div className="inline-note"><Gauge size={17}/><span>Each funded maker first parks its spend in our router (one deposit transaction per wallet, done before the launch). The launch and every maker buy then execute in ONE transaction, tokens straight to each maker: nothing can trade between the launch and your fills, and no window needs tuning. The router becomes the on-chain deployer; leftover deposits are withdrawn automatically. Supports the treasury plus up to 31 makers.</span></div>}
                    </>}
