        ...
        <div className="dialog-body">
            {tab === 'pair' && <div className="form-stack">
                ...
                {draft.mode === 'existing' ? <div className="form-grid two"><Field label="Token address"><input className="mono" value={draft.tokenAddress || ''} onChange={e => set('tokenAddress', e.target.value)} placeholder="0x..."/></Field><Field label={draft.protocol === 'v2' ? 'Bonding curve address' : 'V3 pool address'}><input className="mono" value={draft.poolAddress || ''} onChange={e => set('poolAddress', e.target.value)} placeholder="0x..."/></Field></div> : <>
                    ...
                    {draft.protocol === 'v2' && <>
                        <Field label="Maker buys at launch"><div className="segmented">
                            <button className={(draft.bundleMode || 'none') === 'none' ? 'active' : ''} onClick={() => set('bundleMode', 'none')}>After receipt (direct)</button>
                            <button className={draft.bundleMode === 'window' ? 'active' : ''} onClick={() => set('bundleMode', 'window')}>Bundled · block window</button>
                            <button className={draft.bundleMode === 'atomic' ? 'active' : ''} onClick={() => set('bundleMode', 'atomic')}>Atomic · one transaction</button>
                        </div></Field>
                        {(draft.bundleMode || 'none') === 'none' && <div className="inline-note"><Gauge size={17}/><span>The treasury launches directly on pons; maker buys are broadcast the instant the launch receipt arrives. Snipers can land in the blocks between.</span></div>}
                        {draft.bundleMode === 'window' && <>
                            <div className="form-grid two">
                                <NumberField label="Window (L2 blocks after the launch)" value={draft.bundleMaxBlocks || 3} step={1} min={1} max={50} onChange={v => set('bundleMaxBlocks', Math.max(1, Math.round(v)))}/>
                                <NumberField label="Launch buys (wallets)" value={draft.bundleBuyCount || 10} step={1} min={1} max={31} onChange={v => set('bundleBuyCount', Math.max(1, Math.min(31, Math.round(v))))}/>
                            </div>
                            <div className="inline-note"><Gauge size={17}/><span>Buys are pre-signed against the predicted curve (one per maker, up to the count above). The launch is sent first, then each buy is resubmitted on its own thread until that transaction succeeds or reverts. The router fills a buy only if it lands within the window after the launch block; a late fill reverts and the wallet keeps its ETH. No slippage bound. The router becomes the on-chain deployer.</span></div>
                        </>}
                        {draft.bundleMode === 'atomic' && <div className="inline-note"><Gauge size={17}/><span>Each funded maker first parks its spend in our router (one deposit transaction per wallet, done before the launch). The launch and the snipe-tax-exempt maker buys then execute in ONE transaction, tokens straight to each maker: nothing can trade between the launch and those fills, and no window needs tuning. Extra makers beyond the 32 exemption slots buy later, after the opening tax decays. The router becomes the on-chain deployer; leftover deposits are withdrawn automatically.</span></div>}
                    </>}
            ...
            {tab === 'wallets' && <div className="wallet-assignment">
                <div className="assignment-header"><div><strong>Execution order</strong><p>The first wallet is treasury/deployer; remaining wallets are makers. v2 can only exempt 32 addresses from the opening snipe tax — extra makers still run, and buy after that tax drops to about 1%.</p></div><div className="assignment-actions">
            ...
            {tab === 'execution' && <div className="form-stack">
                ...
                <Field label="Retail response · when profit does not yet cover costs"><div className="segmented">
                    <button className={(draft.retailResponseMode || 'distribute') === 'distribute' ? 'active' : ''} onClick={() => set('retailResponseMode', 'distribute')}>Slow distribution (original)</button>
                    <button className={draft.retailResponseMode === 'target' ? 'active' : ''} disabled={draft.protocol !== 'v2'} onClick={() => set('retailResponseMode', 'target')}>Price target (parameter)</button>
                </div></Field>
                {draft.retailResponseMode === 'target' && <>
                    <NumberField label="Target ratio vs retail average buy price" value={draft.retailTargetRatio ?? 0} step={0.01} min={-0.99} max={10} onChange={v => set('retailTargetRatio', v)}/>
                    <div className="inline-note"><Gauge size={17}/><span>0 = hold: no buy and no sell while any retail still has a net position; pumping resumes only after they are fully flat. Negative (e.g. -0.1) = add holding wallets one by one until selling them pushes the price to the retail average buy price × (1 - 10%), then execute the batch; pump buys pause while retail still holds. Positive (e.g. 0.1) = buy just enough to lift the price to average × (1 + 10%). A profitable retail buy still exits everything immediately.</span></div>
                </>}
...
function WalletVault({state, blocked, notify}: {state: control.VaultState; blocked: boolean; notify: (k: Toast['kind'], t: string) => void}) {
    ...
    const clearWallets = async () => {
        setBusy(true);
        try { await ClearWallets(); setConfirmClear(false); notify('success', 'Trading wallets cleared — funding relays were kept'); }
        catch (e) { notify('error', String(e)); }
    ...
    return <div className="wallet-layout">
        <section className="section-block wallet-table"><div className="section-heading"><div><h2>Wallet inventory</h2><p>{state.wallets?.length || 0} trading accounts · fund routing wallets stay in Settings</p></div><div className="heading-actions"><button className="danger" disabled={busy || blocked || !(state.wallets?.length)} title={blocked ? 'Stop strategies and funding tasks first' : 'Remove imported trading keys so a new set can be imported'} onClick={() => setConfirmClear(true)}><Trash2 size={16}/> Clear all</button><button className="secondary" onClick={lock}><Lock size={16}/> Lock</button></div></div>
            <div className="data-table"><div className="table-head"><span>Label</span><span>Address</span><span>Source</span></div>
                {(state.wallets || []).map(w => <div className="table-row" key={w.id}><div className="name-cell"><span className="wallet-avatar"><WalletCards size={16}/></span><strong>{w.label}</strong></div><span className="mono full-address">{w.address}</span><span className="mode-tag">Encrypted</span></div>)}
                {!(state.wallets || []).length && <Empty text="No trading wallets yet. Import keys here; deposit/withdraw relays are configured in Settings."/>}
            </div>
        </section>
        <section className="section-block import-panel"><div className="section-heading"><div><h2>Import wallets</h2><p>Trading keys only. Fund routing wallets are generated in Settings.</p></div></div><div className="segmented"><button className={mode === 'keys' ? 'active' : ''} onClick={() => setMode('keys')}>Private keys</button><button className={mode === 'mnemonic' ? 'active' : ''} onClick={() => setMode('mnemonic')}>Mnemonic</button></div>
        ...
        {confirmClear && <ConfirmDialog title="Clear trading wallets" message="This permanently deletes imported trading keys from this computer and clears strategy wallet assignments. Deposit/withdraw routing wallets in Settings are kept. You cannot undo this — make sure the trading keys are backed up." confirmLabel="Clear trading wallets" busy={busy} onClose={() => setConfirmClear(false)} onConfirm={clearWallets}/>}
    </div>;
}
...
function SettingsPage({initial, funding, vaultUnlocked, notify}: {
    ...
    return <section className="settings-layout"><div className="section-block"><div className="section-heading"><div><h2>Network</h2><p>Shared defaults for all configured pairs.</p></div></div><Field label="Robinhood Chain RPC"><div className="password-input"><input className="mono" type={showRPC ? 'text' : 'password'} value={settings.rpcEndpoint || ''} onChange={e => setSettings({...settings, rpcEndpoint: e.target.value})} placeholder="wss://..."/><button title={showRPC ? 'Hide endpoint' : 'Reveal endpoint'} onClick={() => setShowRPC(!showRPC)}>{showRPC ? <EyeOff size={17}/> : <Eye size={17}/>}</button></div></Field><div className="inline-note"><Gauge size={17}/><span>WebSocket endpoints enable event-driven pool monitoring. HTTPS falls back to polling.</span></div></div>
        <div className="section-block"><div className="section-heading"><div><h2>Launch router</h2><p>Our PonsMMRouter proxy used by bundled and atomic launches. Leave empty to use the built-in deployment.</p></div></div><Field label="Router contract address"><input className="mono" value={settings.mmRouter || ''} onChange={e => setSettings({...settings, mmRouter: e.target.value})} placeholder="0x1119cDed80b82CA4d732fD1bB20c13f5e9425F60 (default)"/></Field><div className="inline-note"><Gauge size={17}/><span>Bundled launches call launch() here, which records the L2 block via ArbSys and forwards to the official pons factory/router; maker buys use buyAfterLaunch(curve, blocks, wallet) and revert on-chain once the window has passed. Atomic launches use deposit() + launchAndBuyAtomic(). The contract is upgradeable by its owner key.</span></div></div>
